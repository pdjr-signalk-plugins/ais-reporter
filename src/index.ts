...
function reportPositions(app: any, options: any, udpSocket: dgram.Socket) {
  var aisClass: string
    ...
    try {
      if ((new Date(vessel.navigation.position.timestamp)).getTime() > (Date.now() - (options.expiryinterval * 1000))) {
        ...
        if ((msg) && (msg.valid)) {
          ...
          options.endpoints.forEach((endpoint: any) => sendReportMsg(app, udpSocket, msg.nmea, endpoint.ipaddress, endpoint.port));
          count++;
        } else {
  ...
  app.setPluginStatus("Last sent %d position report%s to %d endpoint%s", count, (count == 1)?'':'s', options.endpoints.length, (options.endpoints.length == 1)?'':'s');
}

function reportStaticData(app: any, options: any, udpSocket: dgram.Socket) {
  var aisClass: string
  var aisProperties: any
  var count: number = 0
          ...
          case 'B':
            ...
            if ((msg) && (msg.valid)) {
              ...
              if ((msgB) && (msgB.valid)) {
                ...
                options.endpoints.forEach((endpoint: any) => sendReportMsg(app, udpSocket, msg.nmea, endpoint.ipaddress, endpoint.port));
                options.endpoints.forEach((endpoint: any) => sendReportMsg(app, udpSocket, msgB.nmea, endpoint.ipaddress, endpoint.port));
                count++;
              } else {
...
function sendReportMsg(app: any, udpSocket: dgram.Socket, msg: string, ipaddress: string, port: number) {
  if (udpSocket) {
